...
namespace Application.Application
{
    public class FolhaDePagamentoApp : IFolhaDePagamentoApp
    {
        ...
        public async Task<FolhaDePagamento> ProcessarFolhasDePonto(string file)
        {
            ...
            var registrosAgrupados = _folhaDePontos.GroupBy(x => x.Codigo);

            foreach (var registro in registrosAgrupados)
            {
                var registrosList = registro.ToList();

                var funcionario = new Funcionario();
                funcionario.Codigo = registrosList[0].Codigo;
                funcionario.Nome = registrosList[0].Nome;

                var valorHora = Double.Parse(registrosList[0].ValorHora.Replace("R$ ", "").Replace(" ", ""));
                ...
                foreach (var ponto in registrosList)
                {
                ...
                var debitosDiasFalta = ((diasUteis - diasTrabalhados) * valorHora);
                totalDescontosDepartamento += debitosDiasFalta;
